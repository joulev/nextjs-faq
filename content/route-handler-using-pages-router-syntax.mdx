...
|  | Old `req` | New `req` |
| --- | --- | --- |
| Read JSON body[^4] | `req.body` | `await req.json()`[^5] |
| Read string body | `req.body`<br />([`bodyParser` disabled](https://nextjs.org/docs/pages/building-your-application/routing/api-routes#custom-config)) | `await req.text()`[^6] |
| Read `FormData` body | Third party packages<br />such as [`multer`](https://github.com/expressjs/multer) | `await req.formData()` |
| Read HTTP headers | `req.headers.authorization` | `req.headers.get("authorization")`<br />or use [`headers()`](https://nextjs.org/docs/app/api-reference/functions/headers) |
| Read cookies | `req.cookies` | `req.cookies.get("cookieName")`<br />or use [`cookies()`](https://nextjs.org/docs/app/api-reference/functions/cookies) |
| Read HTTP method | `req.method` | `req.method`<br />though do you need it? |
| Read the<br />request pathname | I don't remember 🥲,[^7]<br />maybe `req.url` | `req.nextUrl.pathname` |
| Read dynamic<br />segment parameters | `req.query` | The [context](https://nextjs.org/docs/app/api-reference/file-conventions/route#context-optional) parameter |
| Read search parameters<br />(query values) | I don't remember 🥲,<br />maybe `req.query` | `req.nextUrl.searchParams` |
...
| Set status code | `res.status(403)` | `return new Response(..., { status: 403 })`<br />`return Response.json({...}, { status: 403 })` |
| Set headers | `res.setHeader("x-hello", "world")` | `return new Response(..., { headers: { "x-hello": "world" } })`<br />`return Response.json({...}, { headers: { "x-hello": "world" } })`[^8] |
| Set cookies | `res.cookie("cookieName", "cookieValue")` | [`cookies().set("cookieName", "cookieValue")`](https://nextjs.org/docs/app/api-reference/functions/cookies) |
| Redirect | `res.redirect(...)` | `return Response.redirect(...)` or [`redirect()`](https://nextjs.org/docs/app/building-your-application/routing/redirecting#redirect-function) |
